...
if [ $2 = "docker" ]; then
    if [ $1 = "before_install" ]; then
        ...
        if [[ "$4" == "python2" ]]; then
            docker run --name notebook -p 8888:8888 -d signetsim/notebook:master-py2 || exit 1;
        else
        ...
        if [[ "$4" == "python2" ]]; then
            docker push signetsim/notebook:master-py2 || exit 1;
        else
            docker push signetsim/notebook:master || exit 1;
            docker tag signetsim/notebook:master signetsim/notebook:master-py3 || exit 1;
            docker push signetsim/notebook:master-py3 || exit 1;
        fi
