...
if [ $2 = "docker" ]; then
    if [ $1 = "before_install" ]; then
        ...
        docker-compose build libsignetsim-notebook || exit 1;
    ...
    if [ $2 = "docker-python3" ]; then
        if [ $1 = "before_install" ]; then
            pip install docker-compose || exit 1;
            docker login -u signetsim -p $3 || exit 1;

        elif [ $1 = "install" ]; then
            docker-compose build libsignetsim-notebook-python3 || exit 1;

        elif [ $1 = "script" ]; then
            docker run --name notebook -p 8888:8888 -d signetsim/notebook:develop-python3 || exit 1;
            WEB_RETURN=`wget -q -O - localhost:8888 | grep \<title\> | cut -d">" -f2 | cut -d"<" -f1 | cut -d" " -f1`
            exit `expr ${WEB_RETURN} != Jupyter`;


        elif [ $1 == "after_script" ]; then
            docker push signetsim/notebook:develop-python3 || exit 1;
        ...
        if [ $1 = "before_install" ]; then
            ...
            if [ "$4" = "python3" ]; then
                ...
                docker exec test_env /bin/bash -c "cd /home/travis/build/vincent-noel/libSigNetSim; bash scripts/install_dep.sh 3" || exit 1;
                ...
                docker exec test_env chown -R www-data:www-data /home/travis/ || exit 1;
                docker exec test_env /bin/bash -c "cd /home/travis/build/vincent-noel/libSigNetSim; bash scripts/install_dep.sh 2" || exit 1;
                docker exec test_env /bin/bash -c "cd /home/travis/build/vincent-noel/libSigNetSim; scripts/install.sh 2" || exit 1;
            fi
