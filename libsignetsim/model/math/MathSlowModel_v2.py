...
class MathSlowModel_v2(object):
	...
	def __init__ (self, parent_model=None, reduced_model=None):
		...
		self.slowStoichiometryMatrix = MathStoichiometryMatrix(self.parentModel)
	...
	def copyVariables(self):
		...
		for variable in self.reducedModel.listOfVariables.values():
		...
		for variable, value in self.reducedModel.solvedInitialConditions.items():
		...
		self.nbOdes = self.reducedModel.nbOdes
		self.nbAssignments = self.reducedModel.nbAssignments
		self.nbConstants = self.reducedModel.nbConstants
		self.nbAlgebraics = self.reducedModel.nbAlgebraics

		self.variablesOdes = []
		for i, var_ode in enumerate(self.reducedModel.variablesOdes):
			t_var = self.listOfVariables.getBySymbol(var_ode.symbol.getSymbol())
			self.variablesOdes.append(t_var)

		self.variablesAssignment = []
		for var_ass in self.reducedModel.variablesAssignment:
		...
		for var_cst in self.reducedModel.variablesConstant:
		...
		for var_alg in self.reducedModel.variablesAlgebraic:
	...
	def findVelocities(self, subs={}, include_fast_reaction=True, include_slow_reaction=True):
		...
		for reaction in self.parentModel.listOfReactions.values():
			# if (reaction.fast and include_fast_reaction) or (not reaction.fast and include_slow_reaction):
				...
				# if any(velocities.applyfunc(self.notZeroFilter)):
	...
	def build(self):
		...
		for i in range(reduced_slow_system.shape[0]):
			t_symbol = self.parentModel.variablesOdes[kept_variables[i]].symbol.getSymbol()
			...
			t_definition.setInternalMathFormula(sum(reduced_slow_system[i,:]))
		...
		for var in self.reducedModel.variablesOdes:
			if var.symbol.getSymbol() not in slow_variables:
		...
		for var, math_formula in self.parentModel.solvedInitialConditions.items():
			if var not in vars:
		...
		for cons_law in self.parentModel.listOfConservationLaws:
		...
		for fast_law in self.fastLaws:
			...
			# system.append(SympyEqual(fast_law.subs(subs), SympyInteger(0)))
