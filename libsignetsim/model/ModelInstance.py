...
class ModelInstance(Model):
	...
	def __init__ (self, model, document):
		...
		self.listOfFunctionDefinitions.copy(self.__mainModel.listOfFunctionDefinitions)#, shift=len(self.listOfFunctionDefinitions))
		self.listOfUnitDefinitions.copy(self.__mainModel.listOfUnitDefinitions)#, shift=len(self.listOfUnitDefinitions))
		self.listOfCompartments.copy(self.__mainModel.listOfCompartments)#, shift=len(self.listOfCompartments))#, subs=var_subs_main_simples)
		self.listOfParameters.copy(self.__mainModel.listOfParameters)#, shift=len(self.listOfParameters), subs=var_subs_main_simples)
		...
		self.listOfInitialAssignments.copy(self.__mainModel.listOfInitialAssignments)#, subs=var_subs_main_simples)
		self.listOfRules.copy(self.__mainModel.listOfRules)#, subs=var_subs_main_simples)
		self.listOfEvents.copy(self.__mainModel.listOfEvents)#, subs=var_subs_main_simples)
		...
		for i, submodel in enumerate(self.__mainModel.listOfSubmodels.values()):
			...
			self.listOfFunctionDefinitions.copy(submodel_instance.listOfFunctionDefinitions, prefix=prefix, subs=var_subs_submodel, deletions=deletions)
			self.listOfUnitDefinitions.copy(submodel_instance.listOfUnitDefinitions, prefix=prefix)
			self.listOfCompartments.copy(submodel_instance.listOfCompartments, prefix=prefix, subs=var_subs_submodel, deletions=deletions, replacements=var_subs_main)
			self.listOfParameters.copy(submodel_instance.listOfParameters, prefix=prefix, subs=var_subs_submodel, deletions=deletions, replacements=var_subs_main)
			self.listOfSpecies.copy(submodel_instance.listOfSpecies, prefix=prefix, subs=var_subs_submodel, deletions=deletions, replacements=var_subs_main)
			self.listOfReactions.copy(submodel_instance.listOfReactions, prefix=prefix, subs=var_subs_submodel, deletions=deletions, replacements=var_subs_main, conversions=conv_factors, extent_conversion=submodel.getExtentConversionFactor(), time_conversion=submodel.getTimeConversionFactor())
			self.listOfInitialAssignments.copy(submodel_instance.listOfInitialAssignments, prefix=prefix, subs=var_subs_submodel, deletions=deletions, replacements=var_subs_main, conversions=conv_factors)
			self.listOfRules.copy(submodel_instance.listOfRules, prefix=prefix, subs=var_subs_submodel, deletions=deletions, replacements=var_subs_main, conversions=conv_factors, time_conversion=submodel.getTimeConversionFactor())
			self.listOfEvents.copy(submodel_instance.listOfEvents, prefix=prefix, subs=var_subs_submodel, deletions=deletions, replacements=var_subs_main, conversions=conv_factors, time_conversion=submodel.getTimeConversionFactor())

			# print "\n > Model's species after submodel's copy: "
